...
class PreviousMap {
  ...
  loadFile(path, cssFile, trusted) {
    if (!trusted && !this.unsafeMap) {
      // Only files explicitly named as `.map` next to the CSS file are safe
      // to read. Anything else could disclose an arbitrary file.
      if (!/\.map$/i.test(path)) return undefined
      if (!cssFile) return undefined
      let relation = relative(dirname(cssFile), path)
      if (
        relation === '..' ||
