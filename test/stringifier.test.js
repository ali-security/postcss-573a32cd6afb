...
test('escapes </style in rule own semicolon', () => {
  let root = parse('@media{a{color:red}}')
  root.first.first.raws.ownSemicolon = '</style>'
  is(root.toString(), '@media{a{color:red}\\3c /style>}')
})

test('escapes </style in child before raw', () => {
  let rule = new Rule({ selector: 'a' })
  let decl = new Declaration({ prop: 'color', value: 'red' })
  decl.raws.before = '\n</style>'
  rule.append(decl)
